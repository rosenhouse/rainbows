...
function geo(S) {
  const V = S.V, R = Math.min(V.w, V.h) * 0.3, e = S.sun * Math.PI / 180;
  return { cx: V.x0 + V.w * 0.6, cy: V.cy - V.h * 0.05, R, ce: Math.cos(e), se: Math.sin(e), e };
}
...
export function draw(g, W, H, S) {
  ...
  const ccw = ((a1 - a0 + Math.PI * 3) % (Math.PI * 2)) > Math.PI;
  g.strokeStyle = 'rgba(234,240,255,.6)'; g.lineWidth = 1;
  g.beginPath(); g.arc(ex[0], ex[1], ar, a0, a1, ccw); g.stroke();
  const am = px([exit[0] - 0.7 * Math.cos(bowAngle(n) / 2 * Math.PI / 180), exit[1] - 0.7 * Math.sin(bowAngle(n) / 2 * Math.PI / 180)], G);
  label(g, bowAngle(n).toFixed(1) + '°', am[0] - 6, am[1], 'right', 'rgba(234,240,255,.9)', MONO);

  // notebook labels
  const e = px(t.entry, G), b = px(t.pts[2], G);
  const sl = px([-2.6, 1.05], G); label(g, 'sunlight', Math.max(14, sl[0]), sl[1], 'left', INK2);
